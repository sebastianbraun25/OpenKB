...
def _routed_acompletion(rules: list[tuple[str, str]]):
    ...
    Returns a single-chunk stream (see ``_mock_response``), matching the
    streaming ``litellm.acompletion`` contract.
    """
...
class TestInsertMode:
    ...
    def _setup_wiki(self, tmp_path):
        ...
        (tmp_path / "raw").mkdir(exist_ok=True)
        (tmp_path / "raw" / "test-doc.pdf").write_bytes(b"fake")
        # Pre-seed both names (see openkb.pending / issue #247) so THIS
        # document's mention is their 3rd — promoting on the first call
        # instead of just buffering another note, matching this class's
        # pre-pending-buffer assumption that "create" writes a page directly.
        _seed_pending(tmp_path, "concepts", "concept-a", "concept-a")
        _seed_pending(tmp_path, "concepts", "concept-b", "concept-b")
        return wiki

    @staticmethod
    def _selective_acompletion(*args, **kwargs):
        """Succeed for "concept-a", raise for "concept-b" — keyed off the
        concept title embedded in the prompt (present in both the pending-
        buffer's note-generation call and the page-generation call, see
        ``_CONCEPT_NOTE_CREATE_USER``/``_CONCEPT_PAGE_USER``) rather than call
        order, since concurrent tasks don't guarantee a fixed completion
        order."""
        ...
        # Carries both "content" (page-generation call) and "note" (the
        # pending-buffer's internal note-generation call) so the SAME
        # response works for either.
        return [
            _mock_response(
                json.dumps({"brief": "b", "content": "# Concept A\n\nBody.", "note": "seen"})
            )
        ]
